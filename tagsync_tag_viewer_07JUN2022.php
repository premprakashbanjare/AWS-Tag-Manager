            ...
            <tbody>
                ...
                if ($result->num_rows > 0) {
                    ...
                    while ($row = $result->fetch_assoc()) {
                        ...
                        echo "<tr id='{$row["id"]}'><td class='tagtype'>{$row['resource_name']}</td><td class='tagtype'>{$row['region']}</td><td class='tagtype'>{$row['resource_type']}</td><td class='tagname'>{$row['compliance']}</td><td><a href=# style='color:#e346d1' onclick='edit_tag_Data($id)'>Edit </a></td></tr>\n";
                    }
                } else {
                    echo "0 results";
                }
